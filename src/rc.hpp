...
// ---------------------------------------------------------------
// RC math helpers (shared CPU/GPU intent)
// ---------------------------------------------------------------
...
// Ray-march interval against scene alpha with front-to-back composition (CPU)
    ...
    for (int x = 0; x < W; ++x) {
      ...
      for (int b = 0; b < 4; ++b) {
        ...
        for (int d = 0; d < 4; ++d) {
          ...
          glm::vec2 bilinearOff = glm::vec2(bilinearIndex * bilinearProbeSize);
          bilinearOff = glm::clamp(
            bilinearOff,
            glm::vec2(0.5f, 0.5f),
            resolution - float(bilinearProbeSize)
          );
          glm::ivec2 bilinearTexel = glm::ivec2(bilinearOff) + bilinearDirCoord;
...
// ---------------------------------------------------------------
// GPU implementation (compute shaders) + accessors
// ---------------------------------------------------------------

class RCGPURenderer {
private:
  GLuint compute_program;    // RC cascade pass
  GLuint clear_program;      // texture clear
  GLuint scene_program;      // scene generation

  GLuint scene_texture, cascade_input, cascade_output;
  int    current_width, current_height;
  bool   gpu_available;

  // RC cascade compute shader (RGBA32F images)
...
void main() {
  ...
  // Tiny compute shader to clear a RGBA32F texture
  const std::string clear_shader_source = R"(
#version 430
layout(local_size_x = 16, local_size_y = 16) in;
...
void main() {
  ...
  imageStore(dst, p, vec4(0.0));
}
)";

  // Scene generation compute shader (replicates scene.hpp on GPU)
  const std::string scene_shader_source = R"(
#version 430
layout(local_size_x = 16, local_size_y = 16) in;
...
void main() {
  ...
  // gl_FragCoord approximation at pixel center with y-flip like CPU path
  vec2 fragCoord = vec2(float(p.x) + 0.5, resolution.y - 0.5 - float(p.y));
  vec4 radiance = vec4(0.0);
  vec2 center   = (resolution * 0.5) - fragCoord;

  // Circle at center
  if (length(center + vec2(0.0)) - circleRadius < 0.0) {
    radiance = circleColor;
  }
  ...
  // Helpers
  static GLuint compile_compute_shader(const std::string& source) {
  ...
  void cleanup() {
    ...
    if (clear_program)   { glDeleteProgram(clear_program);   clear_program   = 0; }
    if (scene_program)   { glDeleteProgram(scene_program);   scene_program   = 0; }
  }
  ...
  void clear_texture_rgba32f(GLuint tex, int width, int height) {
    if (!clear_program) {
      clear_program = compile_compute_shader(clear_shader_source);
    }
    glUseProgram(clear_program);
    glUniform2i(glGetUniformLocation(clear_program, "dims"), width, height);
    glBindImageTexture(0, tex, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
    GLuint gx = (GLuint)((width  + 15) / 16);
    GLuint gy = (GLuint)((height + 15) / 16);
    glDispatchCompute(gx, gy, 1);
    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
  }

  void generate_scene_gpu(glm::vec2 resolution, float circleRadius, const glm::vec4& circleColor) {
    if (!scene_program) {
      scene_program = compile_compute_shader(scene_shader_source);
    }
    glUseProgram(scene_program);
    glUniform2f(glGetUniformLocation(scene_program, "resolution"), resolution.x, resolution.y);
    glUniform1f(glGetUniformLocation(scene_program, "circleRadius"), circleRadius);
    glUniform4f(glGetUniformLocation(scene_program, "circleColor"), circleColor.r, circleColor.g, circleColor.b, circleColor.a);
    glBindImageTexture(0, scene_texture, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
    GLuint gx = (GLuint)((int(resolution.x) + 15) / 16);
    GLuint gy = (GLuint)((int(resolution.y) + 15) / 16);
    glDispatchCompute(gx, gy, 1);
    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
  }

  void upload_scene(const Texture2D& scene) {
    // CPU fallback path: upload a CPU-generated scene
    setup_textures(scene.width(), scene.height());
    std::vector<float> float_data(size_t(scene.width()) * size_t(scene.height()) * 4u);
    for (int y = 0; y < scene.height(); ++y) {
      for (int x = 0; x < scene.width(); ++x) {
        glm::vec4 p = scene.get({x, y});
        size_t idx = (size_t(y) * size_t(scene.width()) + size_t(x)) * 4u;
        float_data[idx + 0] = p.r;
        float_data[idx + 1] = p.g;
        float_data[idx + 2] = p.b;
        float_data[idx + 3] = p.a;
      }
    }
    glBindTexture(GL_TEXTURE_2D, scene_texture);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, scene.width(), scene.height(), GL_RGBA, GL_FLOAT, float_data.data());
  }

  void run_cascade_pass(int baseProbeSize, float baseIntervalLength, int cascadeIndex, glm::vec2 resolution) {
...
public:
  ...
  bool initialize() {
    ...
    if (!GLEW_VERSION_4_3 && !GLEW_ARB_compute_shader) {
      std::cout << "Compute shaders not supported, using CPU fallback\n";
      gpu_available = false;
      return false;
    }
    ...
    clear_program   = compile_compute_shader(clear_shader_source);
    scene_program   = compile_compute_shader(scene_shader_source);
    if (compute_program == 0 || clear_program == 0 || scene_program == 0) {
      std::cout << "Failed to compile compute shaders, using CPU fallback\n";
      gpu_available = false;
      return false;
    }

    gpu_available = true;
    std::cout << "GPU RC renderer initialized successfully\n";
    return true;
  }

  // Run a single RC pass (kept for compatibility; CPU fallback uses it).
  void run_rc_pass(
    int baseProbeSize,
    float baseIntervalLength,
    int cascadeIndex,
    ...
    Texture2D& outTextureN)
  {
    ...
    // For GPU single-pass testing: generate scene on GPU, upload N+1, then run one pass.
    setup_textures(int(resolution.x), int(resolution.y));
    generate_scene_gpu(resolution, 15.0f, glm::vec4(1,1,1,1));

    // Upload cascadeTextureNplus into cascade_input
    std::vector<float> float_data(size_t(cascadeTextureNplus.width()) * size_t(cascadeTextureNplus.height()) * 4u);
    for (int y = 0; y < cascadeTextureNplus.height(); ++y) {
      for (int x = 0; x < cascadeTextureNplus.width(); ++x) {
        glm::vec4 p = cascadeTextureNplus.get({x, y});
        size_t idx = (size_t(y) * size_t(cascadeTextureNplus.width()) + size_t(x)) * 4u;
        float_data[idx + 0] = p.r;
        float_data[idx + 1] = p.g;
        float_data[idx + 2] = p.b;
        float_data[idx + 3] = p.a;
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0,
                    cascadeTextureNplus.width(), cascadeTextureNplus.height(),
                    GL_RGBA, GL_FLOAT, float_data.data());

    run_cascade_pass(baseProbeSize, baseIntervalLength, cascadeIndex, resolution);
    // Result remains in cascade_input after swap.
  }

  // Run the full cascade chain entirely on GPU (preferred path).
  void run_full_rc(
    int baseProbeSize,
    float baseIntervalLength,
    int numCascades,
    glm::vec2 resolution,
    const Texture2D& sceneTexture_UNUSED)
  {
    if (!gpu_available) {
      // CPU fallback: emulate chain
      Texture2D cascadeA(int(resolution.x), int(resolution.y));
      Texture2D cascadeB(int(resolution.x), int(resolution.y));
      ...
      for (int i = numCascades - 1; i >= 0; --i) {
        Texture2D out(int(resolution.x), int(resolution.y));
        run_rc_pass_cpu(baseProbeSize, baseIntervalLength, i, resolution, sceneTexture_UNUSED, *readTex, out);
        if (i != 0) {
          *writeTex = out;
          std::swap(readTex, writeTex);
        } else {
          // last level output could be exposed if needed
        }
      }
      return;
    }

    // GPU path: generate scene on GPU, then cascade
    setup_textures(int(resolution.x), int(resolution.y));
    generate_scene_gpu(resolution, 15.0f, glm::vec4(1,1,1,1));

    // Initialize N+1 (cascade_input) to zeros once
    clear_texture_rgba32f(cascade_input, current_width, current_height);
    ...
    // Ensure final writes are visible to any subsequent read (e.g., copy/stats)
    glMemoryBarrier(GL_ALL_BARRIER_BITS);
  }

  // Accessor to final result texture (after last run_cascade_pass swap)
  GLuint resultTex() const { return cascade_input; }

  // Query availability
  bool gpuAvailable() const { return gpu_available; }
};

// ---------------------------------------------------------------
// Global GPU renderer instance and wrappers
// ---------------------------------------------------------------
...
inline void run_full_rc(
  ...
  const Texture2D& sceneTexture)
{
  g_gpu_renderer.run_full_rc(baseProbeSize, baseIntervalLength, numCascades, resolution, sceneTexture);
}
